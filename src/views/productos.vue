<template>
    <q-layout view="hHh lpR fFf">
        ...
        <q-header height-hint="98" id="headerHome">
            ...
            <q-tabs align="center">
                <q-route-tab to="/createFactura" label="Facturas" />
                            ...
                            <div class="containerModalProductos">
                                ...
                                <div id="formularioProductos">
                                    <div id="columnasProductos">
                                        <q-input outlined v-model="NuevoProducto.codeReference" label="Código de Referencia"
                                            class="custom-input" />

                                        <q-input style="margin-top: 20px; margin-bottom: 20px;" outlined v-model="NuevoProducto.name"
                                            label="Nombre del producto" class="custom-input" />

                                        <q-input outlined type="number" v-model="NuevoProducto.price" label="Precio del producto"
                                            class="custom-input" />
                                    ...
                                    <div id="columnasProductos">
                                        <q-input outlined v-model="NuevoProducto.quantity" label="Cantidad" class="custom-input" />

                                        <q-select outlined v-model="NuevoProducto.unit_measure_id" :options="unitMeasureOptions" label="Unidad de medida" class="custom-input" />
                                    ...
                                    <div id="columnasProductos">
                                        ...
                                        <q-input style="margin-top: 20px; margin-bottom: 20px;" outlined
                                            v-model="NuevoProducto.standard_code_id" label="Codigo Estandar" class="custom-input" />

                                        <q-select outlined v-model="NuevoProducto.tribute_id" :options="tributeOptions" label="Tributo" class="custom-input" />
                                    </div>
                                </div>
...
async function getUnitMeasureOptions() {
    try {
        const response = await getData('/v1/measurement-units?name=');
        if (response && Array.isArray(response)) {
            unitMeasureOptions.value = response.map(item => ({ label: item.name, value: item.id }));
...
async function getTributeOptions() {
    try {
        const response = await getData('v1/tributes/products?name=');
        if (response && Array.isArray(response)) {
            tributeOptions.value = response.map(item => ({ label: item.name, value: item.id }));
...
const postProducto = async () => {
    try {
        ...
        const response = await postDataBackend('items', NuevoProducto.value);
